...
```
8. В файле `synapse-data/homeserver.yaml` добавьте строку `allow_unsafe_locale: true` в блок `database` (на одном уровне с `name: psycopg2`). Измените параметры для безопасности и **отключите федерацию**:
```yaml
...
docker compose ps
```
2. Проверка открытых портов Nginx (должен слушаться порт 443):
```bash
docker port nginx
```
3. Локальная проверка ответа от интерфейса Element (ожидается HTTP/2 200):
```bash
curl -s -k -H "Host: element-ВАШ_ДОМЕН.duckdns.org" -I https://localhost | grep HTTP
```
4. Локальная проверка ответа от сервера Matrix (ожидается JSON со списком поддерживаемых версий протокола):
```bash
curl -s -k -H "Host: matrix-ВАШ_ДОМЕН.duckdns.org" https://localhost/_matrix/client/versions
```
